...
apt install -y ufw git bash-completion nginx exim4 mutt curl procps pigz
...
sed -i '/^## Enterprise-Only Options:/i\cloud:\n  monitoring:\n    free:\n      state: on\n' /etc/mongod.conf
...
rdb = RunDb()
for i in range(10):
    user_name = "user{:02d}".format(i)
    user_mail = "{0}@{0}.{0}".format(user_name)
...
[Service]
Type=simple
ExecStart=${user_home}/net-server/env/bin/gunicorn net_server:app --workers 4 --worker-class uvicorn.workers.UvicornWorker
Restart=on-failure
RestartSec=3
User=${user_name}
